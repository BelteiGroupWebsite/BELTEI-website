...
@section('content')
    <div class="dailyNews mt-4">
        ...
        {{-- Hot News  --}}
        <a href="{{ route('beltei_university.news.detail', $bicc_news->first()->id) }}" class="card card-body d-flex">
            {{-- <a href="{{ route('school.news.bacll-batch18') }}" class="card card-body d-flex"> --}}
            <div class="overflow-hidden">
                @if ($bicc_news->first()->image)
                    @foreach (array_slice(explode(',', $bicc_news->first()->image), 0, 1) as $newsImg)
                        <img class="rounded shadow-sm width-100 w-100" loading="lazy"
                            src="{{ asset('uploaded/university/news/images/' . $bicc_news->first()->id . '/' . $newsImg) }}"
                            alt="News Image">
                    @endforeach
                @endif
            ...
            <div class="p-3">
                <h6 class="Muol-Light line-height-15">
                    @foreach ($bicc_news->first()->newsDetail as $item)
